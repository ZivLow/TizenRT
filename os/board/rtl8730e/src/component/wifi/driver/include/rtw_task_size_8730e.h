...
#define STACK_FOR_IPC_PRINT_WIFI_TASK 626 /* in words: rtw_if_wifi_create_task will *4 */
#define WIFI_SINGLE_TASK_SIZE		                  ((1360 + WTN_SINGLE_TASK_SIZE + NAN_SINGLE_TASK_SIZE + 128 + CONTEXT_SAVE_SIZE) / 4 + STACK_FOR_IPC_PRINT_WIFI_TASK)	/* max 1648 in lite, rtw_if_wifi_create_task: size will *4 */
#else
...
#define STACK_FOR_IPC_PRINT_IPC_DEV 2560 /* in bytes: rtos_task_create direct */
#define WIFI_WHC_IPC_DEV_API_TASK_SIZE            (1280 + 128 + CONTEXT_SAVE_SIZE + STACK_FOR_IPC_PRINT_IPC_DEV)
#else
